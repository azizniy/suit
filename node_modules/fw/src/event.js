...
import {arr, val, vec} from '../index'

export let Machine = (namespace = 'event', debug = false) => {
...
export let listener = (dom, type, callback, flag = false) => {
	var fn = e => {
...
export let support = (element, type) => {
...
export let fileDrop = (dom, t = {}) => {
	var over = listener(dom, 'dragover',  e => {
		t.in && t.in({e})
		leave.on()
		drop.on()
		e.preventDefault()
	})
	var leave = listener(dom, 'dragleave', e => {
		t.out && t.out({e})
		leave.off()
		drop.off()
		e.preventDefault()
	})
	var drop = listener(dom, 'drop', e => {
		t.out && t.out({e})
